...
interface OrderManagementCTAProps {
  title?: string;
  description?: string;
  buttonText?: string;
  buttonLink?: string;
...
export const OrderManagementCTA = ({
    ...
    <section className={`relative isolate overflow-hidden ${isLight ? "bg-[#f6f9fc]" : "bg-[#0A2540]"} py-20`}>
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto px-6 text-center">
        <div className="max-w-4xl mx-auto">
          <h2
            className={`text-[clamp(1.125rem,2.65vw,2.75rem)] sm:text-3xl md:text-4xl lg:text-5xl font-semibold tracking-tight leading-tight whitespace-normal md:whitespace-nowrap ${isLight ? "text-gray-900" : "text-white"}`}
            style={{ marginBottom: "6px" }}
          >
            {title}
          </h2>
          <p className={`text-xl ${isLight ? "text-gray-700" : "text-white"} leading-relaxed mb-4`}>{description}</p>
